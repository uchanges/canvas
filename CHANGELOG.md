...
+ [新增] 平台版画布接入 DEEIX 登录态与云端项目保存，支持跨设备读取并提示保存冲突。
+ [调整] 画布图片、视频和音频改由 DEEIX 文件服务持久化，刷新、切换项目和导出时均按服务端文件恢复。
+ [新增] 画布图片生成与编辑接入 DEEIX 模型目录、异步任务和状态流，支持服务端取消、重试及结果文件直连。
+ [优化] 重新打开画布时会恢复仍在执行的 DEEIX 图片任务，并在服务端完成后写回结果。
+ [新增] 画布文本节点接入 DEEIX 平台模型与流式任务，参考图片改用服务端文件引用。
+ [新增] 画布视频节点接入 DEEIX 视频模型与流式任务，生成结果直接保存为服务端文件引用。
+ [新增] 画布音频节点接入 DEEIX TTS 模型与流式任务，生成结果直接保存为服务端文件引用。
+ [调整] 画布插件 AI Host 改由 DEEIX 统一执行图片、视频、文本和音频任务，支持服务端模型权限、文件引用、计费与审计。
+ [调整] 平台版仅保留 DEEIX 画布入口，隐藏本地 Agent、图片/视频工作台与本地模型配置入口。
+ [修复] 配置节点统一使用 DEEIX 模型目录，修复模型选择与实际调用不一致。
+ [修复] 刷新中断的视频、音频流不再误按图片任务恢复，离开页面前会提示未保存修改。
+ [优化] Docker 支持注入 DEEIX 运行时地址，并支持与 Vite 子路径构建配合部署。
