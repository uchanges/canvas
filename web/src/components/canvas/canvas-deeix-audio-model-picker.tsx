...
import { listDeeixAudioModels, type DeeixAudioModel } from "@/services/api/canvas-audio";
import { cn } from "@/lib/utils";

type CanvasDeeixAudioModelPickerProps = {
    value?: string;
    onChange: (model: string) => void;
    className?: string;
};

export function CanvasDeeixAudioModelPicker({ value, onChange, className }: CanvasDeeixAudioModelPickerProps) {
    const pickerId = useId();
    const [open, setOpen] = useState(false);
    const [models, setModels] = useState<DeeixAudioModel[]>([]);

    useEffect(() => {
        let active = true;
        void listDeeixAudioModels()
            .then((items) => {
