...
import { listDeeixVideoModels, type DeeixVideoModel } from "@/services/api/canvas-video";
import { cn } from "@/lib/utils";

type CanvasDeeixVideoModelPickerProps = {
    value?: string;
    onChange: (model: string) => void;
    className?: string;
};

export function CanvasDeeixVideoModelPicker({ value, onChange, className }: CanvasDeeixVideoModelPickerProps) {
    const pickerId = useId();
    const [open, setOpen] = useState(false);
    const [models, setModels] = useState<DeeixVideoModel[]>([]);

    useEffect(() => {
        let active = true;
        void listDeeixVideoModels()
            .then((items) => {
