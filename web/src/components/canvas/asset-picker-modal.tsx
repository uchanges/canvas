...
export type InsertAssetPayload =
    ...
    | { kind: "image"; dataUrl: string; title: string; fileId?: string; storageKey?: string; width?: number; height?: number; bytes?: number; mimeType?: string }
    | { kind: "video"; url: string; title: string; fileId?: string; storageKey?: string; width?: number; height?: number; bytes?: number; mimeType?: string };
...
function MyAssetsTab({ onInsert }: { onInsert: (payload: InsertAssetPayload) => void }) {
    ...
    const handleInsert = (asset: Asset) => {
        if (asset.kind === "text") {
            ...
            onInsert(
                asset.kind === "video"
                    ? { kind: "video", url: asset.data.url, fileId: asset.data.fileId, storageKey: asset.data.storageKey, title: asset.title, width: asset.data.width, height: asset.data.height, bytes: asset.data.bytes, mimeType: asset.data.mimeType }
                    : { kind: "image", dataUrl: asset.data.dataUrl, fileId: asset.data.fileId, storageKey: asset.data.storageKey, title: asset.title, width: asset.data.width, height: asset.data.height, bytes: asset.data.bytes, mimeType: asset.data.mimeType },
            );
        }
    };
